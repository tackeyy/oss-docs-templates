...
assert_contains "$swift_lint" "run: swiftlint"
assert_contains "$swift_lint" "swift test --enable-code-coverage"
assert_contains "$swift_lint" "swift build --show-bin-path"
assert_contains "$swift_lint" "-name '*.xctest'"
assert_contains "$swift_lint" "no *.xctest bundle"
assert_contains "$swift_lint" "-object"
...
assert_not_contains "$SCRIPT_DIR/lang-configs/swift/workflows/lint.yml" "{{REPO_NAME}}PackageTests"
